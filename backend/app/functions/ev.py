...
def get_chargepoints(n: int = 100):
    ...
    try:
        ...
        # === Create folium map
        map_file = "ev_charging_map_filtered.html"
        ...
        map_ev.save(map_file)
        print(f"✅ Map saved locally as: {map_file}")

        # === Upload HTML map to S3
        S3_BUCKET = 'peanut-butter-project'
        MAP_S3_KEY = f"maps/{map_file}"
        s3 = boto3.client("s3")

        try:
            s3.upload_file(map_file, S3_BUCKET, MAP_S3_KEY)
            map_s3_url = f"https://{S3_BUCKET}.s3.amazonaws.com/{MAP_S3_KEY}"
            print(f"🚀 Uploaded map to S3: {map_s3_url}")
        except Exception as e:
            print(f"❌ Failed to upload map to S3: {e}")
            map_s3_url = None

        # === Upload CSV with centroids to Redshift
        csv_path = "suggested_charging_locations.csv"
        pd.DataFrame(centroids, columns=['latitude', 'longitude']).to_csv(csv_path, index=False)

        S3_CSV_KEY = 'suggested/suggested_charging_locations.csv'
        S3_CSV_PATH = f's3://{S3_BUCKET}/{S3_CSV_KEY}'
        IAM_ROLE_ARN = 'arn:aws:iam::536697253251:role/redshiftreads3'

        s3.upload_file(csv_path, S3_BUCKET, S3_CSV_KEY)
        ...
        # === Final response
        return {
            "message": "Map and suggested locations processed successfully.",
            "map_url": map_s3_url
        }
...
def final_chargepoints(energy_per_charger_kwh: float = 40):
    ...
    # Generate map
    map_url = generate_charger_risk_map(charging_df, combined_df)
...
def generate_charger_risk_map(charging_df, risk_df, output_map="charging_port_risk_map.html"):
    ...
    print(f"✅ Map saved locally to: {output_map}")

    # === Upload to S3
    S3_BUCKET = "peanut-butter-project"
    S3_KEY = f"maps/{output_map}"
    s3 = boto3.client("s3")
